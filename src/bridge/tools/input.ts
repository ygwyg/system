...
export const inputTools: SystemTool[] = [
  {
    name: 'mouse_click',
    description:
      'Click at specific x,y coordinates. Low-level fallback - prefer raycast_search for most tasks.',
    inputSchema: {
      type: 'object',
      properties: {
...
\\""
        ...
        return {
          content: [
            {
              type: 'text',
              text: `Mouse move failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
            },
          ],
          isError: true,
        };
    ...
    inputSchema: {
      ...
      properties: {
        key: {
          type: 'string',
          description:
            'Key to press (e.g., "return", "escape", "tab", "space", "delete", "up", "down", "left", "right")',
        },
        modifiers: {
          type: 'array',
