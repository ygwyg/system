 ...
 */
export const SAFE_SHELL_COMMANDS: Record<
  string,
  {
    description: string;
    allowArgs?: boolean;
...
> = {
  ...
  git: {
    ...
    allowArgs: true,
    argPattern: /^(status|log|diff|branch|remote|show|ls-files|rev-parse)/,
  },
  node: { description: 'Node.js', allowArgs: true },
  npm: {
    description: 'npm',
    allowArgs: true,
    argPattern: /^(list|ls|outdated|version|--version|run|test)/,
  },
 ...
 */
export function validateShellCommand(fullCommand: string): {
  command: string;
  args: string[];
  error?: string;
  useShell?: boolean;
  ...
  const args = parts.slice(1).map(
    (arg) => arg.replace(/^["']|["']$/g, '') // Remove surrounding quotes
  );
  const argsString = parts.slice(1).join(' ');
  ...
  if (config.argPattern && argsString && !config.argPattern.test(argsString)) {
    return {
      command: '',
      args: [],
      error: `Arguments not allowed for "${command}": ${argsString}`,
    };
  }
