...
export const messagingTools: SystemTool[] = [
  {
    name: 'read_imessages',
    description:
      'READ/VIEW iMessages - use this when user wants to see, check, or read messages. Use for: "what did X send", "show messages from X", "read my messages", "last texts from X". Pass phone number directly if known, or contact name to look up.',
    inputSchema: {
      type: 'object',
      properties: {
        from: {
          type: 'string',
          description:
            'Phone number (preferred) or contact name to filter messages. Leave empty for all recent.',
        },
        limit: {
          type: 'number',
        ...
        includeImages: {
          type: 'boolean',
          description: 'Include image attachments in response (default: false)',
        },
      },
    },
    handler: async (args) => {
      ...
      try {
        ...
        if (!existsSync(dbPath)) {
          return {
            content: [
              {
                type: 'text',
                text: 'Messages database not found. Make sure Messages app has been used on this Mac.',
              },
            ],
            isError: true,
          };
        ...
        if (stderr && stderr.includes('unable to open database')) {
          return {
            content: [
              {
                type: 'text',
                text: 'Cannot access Messages database. Grant Full Disk Access to Terminal/the bridge app in System Preferences > Privacy & Security > Full Disk Access.',
              },
            ],
            isError: true,
          };
        ...
        if (!stdout.trim() || stdout.trim() === '[]') {
          return {
            content: [
              {
                type: 'text',
                text: from ? `No messages found from "${from}"` : 'No recent messages found',
              },
            ],
          };
        }
        ...
        const extractTextFromAttributedBody = (hexString: string): string => {
          ...
          try {
            ...
            const idx = buffer.indexOf(nsStringMarker);
            if (idx === -1) return '';
        ...
        try {
          ...
          return {
            content: [{ type: 'text', text: 'Failed to parse messages database response' }],
            isError: true,
          };
        }
            ...
            if (includeImages && imageFiles.length > 0) {
              for (const imgPath of imageFiles.slice(0, 2)) {
                try {
                  ...
                  console.error(`Failed to read attachment at ${imgPath}:`, error);
                }
              }
            }
          }
        ...
        return {
          content: [
            {
              type: 'text',
              text: `Error reading messages: ${error instanceof Error ? error.message : 'Unknown'}. Make sure Full Disk Access is granted in System Preferences > Privacy.`,
            },
          ],
          isError: true,
        };
  ...
  },
  {
    name: 'send_imessage',
    description:
      'SEND iMessage - use this when user wants to send/text someone. Pass phone number directly if known, or contact name to look up.',
    inputSchema: {
      type: 'object',
      properties: {
        to: {
          type: 'string',
          description: 'Phone number (preferred) or contact name of the recipient',
        },
    ...
    handler: async (args) => {
      ...
      if (!to || !message) {
        return {
          content: [{ type: 'text', text: 'Error: Both "to" and "message" are required' }],
          isError: true,
        };
      }
      ...
      try {
          ...
          try {
            ...
            if (phoneResult && phoneResult.trim()) {
              ...
              return {
                content: [
                  {
                    type: 'text',
                    text: `Could not find phone number for contact "${args.to}". Try using a phone number directly.`,
                  },
                ],
                isError: true,
              };
            ...
            return {
              content: [
                {
                  type: 'text',
                  text: `Could not look up contact "${args.to}". Try using a phone number directly.`,
                },
              ],
              isError: true,
            };
        ...
        return {
          content: [
            { type: 'text', text: `Message sent to ${args.to}${to !== args.to ? ` (${to})` : ''}` },
          ],
        };
      } catch (error) {
        return {
          content: [
            {
              type: 'text',
              text: `Error sending message: ${error instanceof Error ? error.message : 'Unknown'}`,
            },
          ],
          isError: true,
        };
  ...
  },
  {
    ...
    description:
      "Search for contacts by name to find their phone number or email. Pass the person's actual name.",
    inputSchema: {
      type: 'object',
      properties: {
        query: {
          type: 'string',
          description: 'Name to search for (e.g., "John Smith", "Mom", "Jane")',
        },
      ...
      try {
        ...
        return {
          content: [
            {
              type: 'text',
              text: `Error searching contacts: ${error instanceof Error ? error.message : 'Unknown'}`,
            },
          ],
          isError: true,
        };
