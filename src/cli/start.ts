...
function showStatus(services: Service[], _config: Config) {
  console.clear();
  log(LOGO);
  log('');
...
function showPermissionWarning() {
  ...
  log(
    `${c.yellow}│${c.reset} ${c.yellow}⚠${c.reset}  ${c.white}Full Disk Access not granted${c.reset}                         ${c.yellow}│${c.reset}`
  );
  log(
    `${c.yellow}│${c.reset}                                                         ${c.yellow}│${c.reset}`
  );
  log(
    `${c.yellow}│${c.reset}  iMessage reading won't work until you:                 ${c.yellow}│${c.reset}`
  );
  log(
    `${c.yellow}│${c.reset}                                                         ${c.yellow}│${c.reset}`
  );
  log(
    `${c.yellow}│${c.reset}  1. Open ${c.cyan}System Settings → Privacy & Security${c.reset}          ${c.yellow}│${c.reset}`
  );
  log(
    `${c.yellow}│${c.reset}  2. Click ${c.cyan}Full Disk Access${c.reset}                             ${c.yellow}│${c.reset}`
  );
  log(
    `${c.yellow}│${c.reset}  3. Add your terminal app and enable it                 ${c.yellow}│${c.reset}`
  );
  log(
    `${c.yellow}│${c.reset}  4. Restart this terminal                               ${c.yellow}│${c.reset}`
  );
  log(
    `${c.yellow}│${c.reset}                                                         ${c.yellow}│${c.reset}`
  );
  log(
    `${c.yellow}│${c.reset}  Run ${c.cyan}npm run check${c.reset} for detailed permission status      ${c.yellow}│${c.reset}`
  );
  log(`${c.yellow}└─────────────────────────────────────────────────────────┘${c.reset}`);
  log('');
}
...
async function main() {
  ...
  // Build services list
  const services: Service[] = [{ name: 'Bridge', status: 'starting' }];
